...
void SetLevel(Block blocks[BLOCK_ROWS][BLOCK_COLS], int level) {
    for (int i = 0; i < BLOCK_ROWS; i++) {
        for (int j = 0; j < BLOCK_COLS; j++) {
            blocks[i][j].active = 1;  // **Pastikan blok aktif**
            
            switch (level) {
                case 1:
                    blocks[i][j].color = (Color){139, 69, 19, 255};  // Warna kayu
                    blocks[i][j].durability = 1;
                    break;
                case 2:
                    if (i < BLOCK_ROWS / 2) {
                        blocks[i][j].color = (Color){255, 0, 0, 255};  // Warna batu bata
                ...
                case 3:
                    if (i < BLOCK_ROWS / 3) {
                        blocks[i][j].color = (Color){105, 105, 105, 255};  // Warna besi
                    ...
                    break;
                default:
                    blocks[i][j].color = (Color){139, 69, 19, 255};  // Default ke kayu
                    blocks[i][j].durability = 1;
                    break;
            }

            printf("Block[%d][%d]: Active = %d, Warna = (%d, %d, %d, %d), Durability = %d\n",
                   i, j, blocks[i][j].active, 
                   blocks[i][j].color.r, blocks[i][j].color.g, 
                   blocks[i][j].color.b, blocks[i][j].color.a, 
