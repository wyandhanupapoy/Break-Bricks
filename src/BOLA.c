        ...
        if (*state == GAME_PLAY || *state == GAME_START) {
            ...
            while (blockNode != NULL) {
                if (blockNode->data.active && 
                    CheckCollisionCircleRec(curr->position, curr->radius, blockNode->data.rect)) {
            ...
            if (closestBlockNode != NULL) {
                ...
                if (closestBlockNode->data.hitPoints <= 0) {
                    ...
                    if (GetRandomValue(0, 100) < 30) {
                        ...
                        AddPowerUp(powerUpList, type, (Vector2){
                            ...
                            closestBlockNode->data.rect.y
                        });
                    }    
                } else {
                    closestBlockNode->data.color = (closestBlockNode->data.hitPoints == 2) ?
                ...
                Vector2 normal = {0};
                float ratioX = fabsf(ballCenter.x - blockCenter.x) / closestBlockNode->data.rect.width;
                float ratioY = fabsf(ballCenter.y - blockCenter.y) / closestBlockNode->data.rect.height;
                
                if (ratioX > ratioY) {
                    normal.x = (ballCenter.x < blockCenter.x) ? -1 : 1; // Horizontal
                } else {
                    normal.y = (ballCenter.y < blockCenter.y) ? -1 : 1; // Vertikal
                }
