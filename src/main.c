...
void DrawActivePowerUpTimers(PaddleList *pList) { // Ubah nama parameter agar tidak bentrok
    if (!pList || !pList->head) {
        return;
    }
...
void DrawDynamicLevelBackground(int level, LevelBackgroundState *bgState) {
    ...
    if (!bgState && (level == 2 || level == 3)) { // Perlu pengecekan bgState != NULL
    ...
    switch (level) {
        ...
        case 2: // Latar belakang dengan sedikit animasi
            if (bgState) { // Pastikan bgState tidak NULL
                for (int y = 0; y < SCREEN_HEIGHT; y++) {
...
int main() {
    ...
    if (icon.data != NULL) SetWindowIcon(icon); // Hanya jika berhasil load
    else TraceLog(LOG_WARNING, "MAIN: Failed to load window icon.");
    ...
    while (!WindowShouldClose()) {
        ...
        if (gameState == GAME_MENU) {
            ...
            if(stopwatchList) { // Hapus stopwatch lama jika ada
            ...
            if (IsStartGame()) {
                ...
                if (level > 0 && level <=3) { // Pastikan level valid
                    currentLevel = level;
        ...
        if (IsKeyPressed(KEY_P) && (gameState == GAME_PLAY || gameState == GAME_START)) { // Pause bisa juga di GAME_START
            isPaused = !isPaused;
            if (isPaused) PauseMusic();
            else PauseMusic(); // Gunakan ResumeMusic agar melanjutkan, bukan toggle play/stop
        }
        ...
        if (!isPaused) {
            switch (gameState) {
                ...
                case GAME_PLAY:
                    ...
                    if(stopwatchList) UpdateStopwatch(stopwatchList);
                    ...
                    if (AllBlocksDestroyed(&blockList) && gameState != GAME_WIN) { // Cek gameState agar tidak memicu ulang
                        gameState = GAME_WIN;
                        PlaySfx("game_win");
                        ChangeMusic("assets/sounds/background_music.mp3");
                    }
        ...
        BeginDrawing();
            ...
            if (gameState == GAME_MENU) {
                ...
                if (gameState == GAME_PLAY || gameState == GAME_START || gameState == GAME_OVER || gameState == GAME_WIN) {
                    ...
                    if (stopwatchList) DrawStopwatch(stopwatchList);
            ...
            if (lifeLostThisFrame && (gameState == GAME_START)) { // Tampilkan hanya di state start setelah nyawa hilang
    ...
    UnloadSoundSystem();
    UnloadMedalTextures();
    if(icon.data != NULL) UnloadImage(icon); // Hanya unload jika berhasil load
    CleanupBackground();
    CloseWindow();
    return 0;
}
