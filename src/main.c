...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_W)) IncreaseVolume(); // Assuming W for volume up
        if (IsKeyPressed(KEY_S)) DecreaseVolume(); // Assuming S for volume down
        ...
        if (gameState == GAME_MENU) {
            ...
            if (IsStartGame()) {
                ...
                if (level > 0) {
                    ...
                    InitBola(&bolaList);
                    InitLivesSystem(3);
                    if (stopwatchList) { /* Free old if any, though unlikely here */ }
        ...
        if (IsKeyPressed(KEY_P) && gameState == GAME_PLAY) {
            ...
            else PauseMusic(); // Assuming backgroundMusic is the stream
        }
        ...
        if (!isPaused) {
            switch (gameState) {
                case GAME_START:
                    ...
                    if (IsKeyPressed(KEY_SPACE)) { // Launch ball
                        lifeLost = false; // Reset life lost visual
                        if (bolaList.head != NULL) { // Ensure ball exists before launching
                           // Initial speed might be set in InitBola or here upon launch
                           // If ball speed y is 0, set it to launch upwards
                           if(bolaList.head->speed.y == 0) bolaList.head->speed.y = -6.0f;
                           if(bolaList.head->speed.x == 0 && paddleList.head != NULL) { // Launch with some horizontal influence from paddle
                ...
                case GAME_PLAY:
                    ...
                    if (SemuaBolaMati(&bolaList)) {
                        ...
                        if (!HasLivesLeft()) {
                            ...
                            if(paddleList.head) { // If paddle exists
                                ActivePowerUp *currEffect = paddleList.head->activePowerUps;
                                while(currEffect) {
                                    if(currEffect->type == POWERUP_LONG_PADDLE) paddleList.head->rect.width = PADDLE_WIDTH_DEFAULT;
                            ...
                            FreePowerUp(&powerUpList); // Clear falling powerups
                ...
                case GAME_OVER:
                    ...
                    if (gameEndTimer >= returnDelay || IsKeyPressed(KEY_R)) { // R to return to menu
                        gameState = GAME_MENU;
                        gameEndTimer = 0.0f;
                    }
                    break;
        ...
        if (gameState == GAME_MENU) {
            ...
            if (gameState == GAME_PLAY || gameState == GAME_START || gameState == GAME_OVER || gameState == GAME_WIN) {
                ...
                if (stopwatchList) DrawStopwatch(stopwatchList); // Check if stopwatchList is initialized
        ...
        if (lifeLost && (gameState == GAME_START || gameState == GAME_PLAY)) { // Show only during gameplay/start after life lost
            DrawText("LIFE LOST!", SCREEN_WIDTH / 2 - MeasureText("LIFE LOST!", 50)/2, SCREEN_HEIGHT / 2 - 50, 50, RED);
        }

        // Game state specific screens on top
        if (gameState == GAME_START && !lifeLost) { // Don't show if "LIFE LOST!" is showing
    ...
    if (stopwatchList) { // Assuming stopwatchList is a single allocated struct or head of a list needing freeing.
